...
saveRDS(chr_name, file = paste('chr_names/name_mds_CHR',as.character(opt$Chr), '_fdr', as.character(100*opt$Fdr), '.rds', sep=''))
cat('Chr names saved. \n')
...
cutoff.mds = find.cutoff(opt$Fdr, sort.med.score)
...
saveRDS(S1.list, file = paste('selection/CHR',as.character(opt$Chr), '_MDS', as.character(opt$MdsTime), '_fdr', as.character(100*opt$Fdr),'.rds', sep=''))